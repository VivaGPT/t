...
fake = Faker('nl_NL')
def generate_sweden_phone_number():
    # Код страны Швеции
    country_code = "+46"
    # Код города, выбирается случайным образом
    city_code = random.choice(["8", "18", "21", "31", "42", "44", "46", "54", "60", "90"])
    # Номер телефона, выбирается случайным образом
    phone_number = "".join([str(random.randint(0, 9)) for _ in range(8)])
    # Объединяем все части номера телефона вместе и удаляем пробелы
...
class Register():
    ...
    def registration(self, json_data_generate):
        ...
        print(generate_sweden_phone_number())
        json_data = {
            "company": fake.bs(),
            "country": "SE",
            "phonenumber":  str(generate_sweden_phone_number()),
...
# }
    ...
    def adgroup_create_page(self, id, campaign_id, pixel_d, name, country, external_action, start_time, budget):
            ...
            if response.json()['msg'] == 'success':
                ...
                print('None')
                return None
    def adgroup_create_page_check(self, id, campaign_id, pixel_d, name, country, external_action, start_time, budget):
            seed = random.randint(0,2000)
            cookies = self.COOKIES
            headers = {
            'Connection': 'keep-alive',
            ...
            response = requests.post('https://ads.tiktok.com/api/v2/i18n/perf/ad/check/?aadvid='+id+'&req_src=ad_creation&msToken='+cookies['msToken']+'=&X-Bogus=DFplmt'+str(seed)+'&_signature='+str(seed), cookies=cookies, headers=headers, verify=True, json=json_data,proxies=self.PROXY,timeout=50)
            print(response.json())
            if response.json()['msg'] == 'success':
                cc = str((response.json()['data']['fake_ad_ids']))
...
def card_bind(id, ticket, nonce, agreement_id, cardId, card_data, proxy):
  ...
  "must_do_3ds":True
    ...
    d = 'merchant_id=10202007Bym6R6&biz_content=' + \
...
##########################################################
    cookies_path_file = "dolphin-anty-cookies-278.txt"
    ...
    while count <= fine:
        try:
            ...
            adgroup_id2_2 = b.adgroup_create_page(
                id, campaign_id, PixelID, "DE_"+id+'_1', 2921044, '5', '2023-02-19 20:15:02', '300.00')    #####!!!!!!!!!!!! 5
             
            fake_ad_ids = b.adgroup_create_page_check(
                id, campaign_id, PixelID, "DE_CHECK"+id+'_1', 2921044, '5', '2023-02-19 20:15:02', '300.00')    #####!!!!!!!!!!!! 5
           ...
           # adgroup_id2_2 = b.adgroup_create(id, campaign_id, PixelID, "Spain_2", "2510769")
            ...
            site_url = "https://mobi-power.website/&id="+str(id) #tg://join?invite=+0CcUQ__Nu9I3OTdi
